...
export const VisualTracePlayer: React.FC<VisualTracePlayerProps> = ({ trace, onHighlightLine, onApplyCode }) => {
  ...
  // Resolve Hash Maps / Dictionaries in variables (e.g. seen: { 2: 0, 7: 1 })
  ...
  // Resolve Stacks / Queues in variables (e.g. stack: [1, 2, 3])
  ...
  }, [currentStepIndex, currentStep?.line, onHighlightLine]);
  ...
  return (
    <div className="my-3 rounded-lg border-2.5 border-black bg-[#fffdfa] shadow-[4px_4px_0px_#000] overflow-hidden text-black select-none">
      {/* 1. Header Bar */}
      <div className="flex items-center justify-between px-3 py-2 bg-[#ffe600] border-b-2 border-black text-black">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded bg-black text-[#ffe600] flex items-center justify-center font-black text-xs border border-black shadow-[1px_1px_0px_#000]">
            <Activity className="w-3 h-3 stroke-[3]" />
          </div>
          <div>
            <span className="font-black text-xs tracking-wide">
              {trace.title || "VISUAL CODE TRACER"}
            </span>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <div className="bg-white text-black px-2 py-0.5 rounded text-[10px] font-mono font-black border border-black shadow-[1px_1px_0px_#000]">
          ...
          <button
            ...
            className="neo-btn bg-white hover:bg-neutral-100 text-black px-1.5 py-0.5 rounded text-[10px] font-black border border-black shadow-[1px_1px_0px_#000] cursor-pointer"
          >
            {playbackSpeed}x
          </button>
        </div>
      ...
      <div className="px-3 pt-2.5 pb-1.5 bg-[#f6f3eb] border-b border-black/20">
        <div className="flex items-center justify-between text-[9px] font-mono font-bold text-neutral-500 mb-1">
        ...
        <input
          ...
          className="w-full h-2 bg-white rounded-lg appearance-none cursor-pointer border border-black accent-[#00f0ff] shadow-[1px_1px_0px_#000]"
        />
      </div>

      {/* 3. Main Step Visual Display */}
      <div className="p-3 space-y-2.5">
        {/* Active Line Execution Banner */}
        {currentStep.code && (
          <div className="rounded border-2 border-black bg-[#1e1e1e] text-white p-2.5 shadow-[2px_2px_0px_#000]">
            <div className="flex items-center justify-between text-[10px] text-neutral-400 font-mono mb-1 pb-1 border-b border-neutral-700">
              <span className="flex items-center gap-1 text-[#00f0ff] font-bold">
                ...
                {currentStep.line ? `Line ${currentStep.line}` : "Current Execution"}
              </span>
        ...
        {currentStep.explanation && (
          <div className="p-2.5 rounded border-2 border-black bg-[#e0f7fa] text-xs shadow-[2px_2px_0px_#000]">
            <div className="flex items-center gap-1.5 font-black text-[11px] text-[#006064] uppercase mb-1">
        ...
        {/* 🌟 Interactive Array & Pointer Visualizer Box */}
        {arrayData && arrayData.elements && arrayData.elements.length > 0 && (
          <div className="rounded border-2 border-black bg-white overflow-hidden shadow-[2px_2px_0px_#000]">
            <div className="flex items-center justify-between px-2.5 py-1.5 bg-[#ffe600] border-b-2 border-black text-[10px] font-black uppercase text-black">
                ...
                {arrayData.elements.map((elem, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center min-w-[44px]">
                      {/* Value Box */}
                      <div
                        className={`w-11 h-11 rounded-lg border-2 border-black flex items-center justify-center font-mono text-xs font-black transition-all shadow-[1.5px_1.5px_0px_#000] ${
                          isHighlighted
                      ...
                      {/* Index Label */}
                      <div className="text-[9px] font-mono font-bold text-neutral-500 mt-1">
                        [{idx}]
                      </div>

                      {/* Moving Pointer Indicator */}
                      <div className="flex flex-col items-center gap-0.5 mt-0.5 min-h-[26px]">
                        {matchingPointers.map((ptr) => (
                          <div
                            key={ptr.name}
                            className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-1 duration-150"
                          >
                            <span className="text-[9px] leading-none text-black font-black">▲</span>
                            <span
        ...
        {/* 🌟 Interactive Hash Map / Set Visualizer */}
        {hashMaps.length > 0 && (
          <div className="space-y-2">
            {hashMaps.map((map) => (
              <div
                key={map.name}
                className="rounded border-2 border-black bg-white overflow-hidden shadow-[2px_2px_0px_#000]"
              >
                <div className="flex items-center justify-between px-2.5 py-1.5 bg-[#00f0ff] border-b-2 border-black text-[10px] font-black uppercase text-black">
                  <div className="flex items-center gap-1.5">
        ...
        {/* 🌟 Interactive Stack / Queue Visualizer */}
          ...
          <div className="space-y-2">
            {stackQueues.map((st) => (
              <div
                ...
                className="rounded border-2 border-black bg-white overflow-hidden shadow-[2px_2px_0px_#000]"
              >
                <div className="flex items-center justify-between px-2.5 py-1.5 bg-[#4ade80] border-b-2 border-black text-[10px] font-black uppercase text-black">
                  <div className="flex items-center gap-1.5">
                    <Layers className="w-3.5 h-3.5 stroke-[2.5]" />
                ...
                <div className="p-2.5 overflow-x-auto bg-[#faf8f5]">
                  <div className="flex items-center gap-1.5 min-w-max">
                    ...
                    {st.elements.map((el, i) => (
                      <div
                        key={i}
                        className="px-2.5 py-1.5 rounded-md border-2 border-black bg-white shadow-[1.5px_1.5px_0px_#000] font-mono text-xs font-black"
                      >
                        {String(el)}
                      </div>
                    ))}
        ...
        {currentStep.variables && Object.keys(currentStep.variables).length > 0 && (
          <div className="rounded border-2 border-black bg-white overflow-hidden shadow-[2px_2px_0px_#000]">
            <div className="flex items-center justify-between px-2.5 py-1 bg-[#f0ede6] border-b border-black text-[10px] font-black uppercase">
              <span className="flex items-center gap-1">
                <Layers className="w-3 h-3" /> Live Variables
              </span>
              <span className="font-mono text-neutral-500">
                {Object.keys(currentStep.variables).length} active
              </span>
            </div>
            <div className="p-2 grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-36 overflow-y-auto font-mono text-xs">
              {Object.entries(currentStep.variables).map(([varName, val]) => {
                ...
                return (
                  <div
                    key={varName}
                    className={`p-1.5 rounded border border-black flex flex-col justify-between transition-colors ${
                      hasChanged
                        ? "bg-[#ffe600] font-black shadow-[1.5px_1.5px_0px_#000]"
                        : "bg-neutral-50"
                    }`}
                  >
                    <span className="text-[10px] text-neutral-600 font-bold uppercase truncate">
                      {varName}
                    </span>
                    <span className="font-mono text-xs text-black font-black truncate mt-0.5">
                      {strVal}
                    </span>
                  </div>
                );
        ...
        {/* Live Console Stdout Stream */}
        {currentStep.output && (
          <div className="rounded border-2 border-black bg-[#121212] text-neutral-200 p-2 shadow-[2px_2px_0px_#000] font-mono text-[11px]">
      ...
      {/* 4. Playback Controller Toolbar */}
      <div className="px-3 py-2 bg-[#f0ede6] border-t-2 border-black flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          {/* First Step */}
          <button
            type="button"
            ...
            title="First Step"
            className="neo-btn bg-white hover:bg-neutral-100 disabled:opacity-40 p-1.5 rounded border border-black shadow-[1px_1px_0px_#000] cursor-pointer"
          >
          ...
          <button
            ...
            disabled={currentStepIndex === 0}
            title="Previous Step"
            className="neo-btn bg-white hover:bg-neutral-100 disabled:opacity-40 px-2 py-1 rounded text-xs font-black border border-black shadow-[1px_1px_0px_#000] flex items-center gap-1 cursor-pointer"
          >
        ...
        <button
          ...
          className={`neo-btn px-4 py-1.5 rounded text-xs font-black border-2 border-black shadow-[2px_2px_0px_#000] flex items-center gap-1.5 cursor-pointer text-black transition-transform active:translate-x-0.5 active:translate-y-0.5 ${
